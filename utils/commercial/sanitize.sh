...
files=`find ${path} -name "*.h" -o -name "*.y" -o -name "*.in" -o -name "*.c" \
  -o -name "*.f" -o -name "*.F" -o -name "*.l" -o -name "*.texi" -o -name "Makefile*"`

for A in $files; do
  echo "cleaning up \"$A\""
  mv $A $A.old
  sed -e "/# This program is free software; you can redistribute/,/USA./D"\
      -e '/# Copyright (C).*Geuzaine/a\
#\
