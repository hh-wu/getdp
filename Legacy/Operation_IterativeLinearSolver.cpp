...
//Numer. Linear Algebra Appl., 1998
	...
	for(int cpt_deflation = 0; cpt_deflation < nb_deflation; cpt_deflation ++){		 
		PetscScalar vec_temp[n];
		ierr = VecGetValues(DeflationVec[cpt_deflation], n, ix, vec_temp);CHKERRQ(ierr);
		ierr = MatSetValues(U, n, ix, 1, &cpt_deflation, vec_temp, ADD_VALUES);CHKERRQ(ierr);
	}
	...
	for (int cpt_deflation =0; cpt_deflation <nb_deflation; cpt_deflation ++){
		...
		ierr = VecGetValues(Y, n, ix, vec_temp);CHKERRQ(ierr);
		ierr = MatSetValues(AU, n, ix, 1, &cpt_deflation, vec_temp, INSERT_VALUES);CHKERRQ(ierr);
	}
...
PetscErrorCode BuildIterationMatrix(Mat A, Vec X, Mat *IterationMatrix)
{
	...
	for(int cpt=0;cpt<n;cpt++)
	{
		...
		ierr = VecGetValues(Aej, n, ix, vec_temp);CHKERRQ(ierr);
		ierr = MatSetValues(AuxMatrix, n, ix, 1, &cpt, vec_temp, INSERT_VALUES);CHKERRQ(ierr);
	}
