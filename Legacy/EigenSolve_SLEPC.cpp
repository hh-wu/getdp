                       ...
                       double shift_r, double shift_i)
{
  ...
  if (nconv > 0) {
    ...
    for (int i = 0; i < nconv; i++){
      ...
      double ore = sqrt(abs) * cos(0.5*arg);
      double oim = sqrt(abs) * sin(0.5*arg);
      double fre = ore / 2. / M_PI;
      double fim = oim / 2. / M_PI;
                      ...
                      double shift_r, double shift_i)
{
  ...
  // GenerateSeparate provides: -w^2 M3 x + iw M2 x + M1 x =
  // b. Flag_Init[i] is set if Mi exists, so check if we have enough
  // data:
