...
extern PetscErrorCode MatMultFieldMat(Mat A, Vec X, Vec Y);
extern PetscErrorCode STD_vector_to_PETSc_Vec(	std::vector<std::vector<std::vector<double> > > std_vec,
												Vec petsc_vec);
extern PetscErrorCode PETSc_Vec_to_STD_Vec(	Vec 											petsc_vec,
											std::vector<PetscInt>							FieldSizes,
											std::vector<std::vector<std::vector<double> > > *std_vec);
extern PetscErrorCode CreateFieldMat(FieldMat **shell);
extern PetscErrorCode SetFieldMat(FieldMat 				**shell,
								  std::vector<PetscInt> FieldIndices,
								  std::vector<PetscInt> FieldSizes,
								  struct Resolution  	*Resolution_P,
								  struct Operation   	*Operation_P,
