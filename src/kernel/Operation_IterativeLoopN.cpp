                         ...
                         List_T *xPrevious_L, List_T *PostOpSolutionPrevious_L)
{
  ...
  for(int i = 0; i < List_Nbr(ILsystems_L); i++) {
    ...
    // Current.Residual = ErrorRatio; //kj+++ (should be commented here,
    // Current.ResidualN used instead and defined at the end of
    // CalcMaxErrorRatio)
  ...
  Current.ResidualN =
    MaxErrorRatio; // kj+++ Residual computed here for IterativeLoopN
                              ...
                              int *Flag_Break)
{
  ...
  if(Num_Iteration > NbrMaxIteration) {
    ...
    Flag_IterativeLoopConverged = 0;
    // Message::Info(3, "IterativeLoopN did NOT converge (%d iterations, error
    // ratio %g)", //kj+++ (warning is better)
    Message::Warning(
