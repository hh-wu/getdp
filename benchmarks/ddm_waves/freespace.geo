...
For ii In {start+1:end+1}
  ...
  If (ii!=iPoint)
    ...
    Point(1) = {(ii-1)*Ddom+shiftX,-dGeo+shiftY,0,lc};
    Point(2) = {ii*Ddom+shiftX,-dGeo+shiftY,0,lc};
    Point(3) = {ii*Ddom+shiftX,0+shiftY,0,lc};
    Point(4) = {(ii-1)*Ddom+shiftX,0+shiftY,0,lc};

    Point(7) = {ii*Ddom+shiftX,ySource+0*shiftY,0,lc};
    Point(9) = {(ii-1)*Ddom+shiftX,ySource+0*shiftY,0,lc};

    Point(52) = {(ii)*Ddom+dBb+shiftX,-dGeo+shiftY,0,lc};
    Point(57) = {(ii)*Ddom+dBb+shiftX,ySource+0*shiftY,0,lc};
    Point(53) = {(ii)*Ddom+dBb+shiftX,0+shiftY,0,lc};

    Point(51) = {(ii-1)*Ddom-dBb+shiftX,-dGeo+shiftY,0,lc};
    Point(59) = {(ii-1)*Ddom-dBb+shiftX,ySource+0*shiftY,0,lc};
    Point(54) = {(ii-1)*Ddom-dBb+shiftX,0+shiftY,0,lc};
  ...
  If (ii == iPoint)
    ...
    Transfinite Line{10,15,21} = (xSource-(ii-1)*Ddom-shiftX)/lc+1 Using Progression 1;
    Transfinite Line{11,14,19} = (ii*Ddom-xSource+shiftX)/lc+1 Using Progression 1;
  ...
  If(StrCmp(OnelabAction, "check")) // only mesh if not in onelab check mode
    Printf("Meshing waveguide subdomain %g...", i-1);
    Mesh 2 ;
    CreateDir Str(DIR);
