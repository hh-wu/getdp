...
// moving band
For i In {NbrSect:NbrSectTot-1}
If (i < NbrSectTot)
  For j In {0:1}
    dP=newp-1;

    Point(dP+4)  = {ii_x, ii_y,0, pMB};//.7e-3
    Point(dP+16) = {0, l_y, 0, pMB};//.3e-3
    Point(dP+18) = {oo_x, oo_y,0, pMB};//.6e-3
    Rotate {{0,0,1},{0,0,0}, RotorAngle_R+2*Pi*i/NbrSectTot} { Point{dP+4}; Point{dP+16}; Point{dP+18}; }
    If (j==1)
      Symmetry {Cos(RotorAngle_S+2*Pi*i/NbrSectTot),Sin(RotorAngle_S+2*Pi*i/NbrSectTot),0,0}
...
For k In {0:NbrPolesTot-1}
  Physical Line(ROTOR_BND_MOVING_BAND+k) = {InnerMB_[{k*4*NbrSect/NbrPoles:(k*4+4)*NbrSect/NbrPoles-1}]};
EndFor
