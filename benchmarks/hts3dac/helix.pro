Group {
  ...
  Air = Region[3001];
  Matrix = Region[3002];
  Filaments = Region[3003];
  ExternalBnd = Region[2011];
  MatrixBnd = Region[2012];
  BottomBnd = Region[{2021, 2022, 2023}];
  BottomBnd = Region[{2031, 2032, 2033}];

  Omega_c = Region[{Matrix,Filaments}];
  Omega_a = Region[{Air}];
  ...
  DefineConstant[ Cut2TO = {2007, Min 1, Step 1,
      Name "Cohomology/H^1(M_a)/Cut 2 for TO formulation"} ];
