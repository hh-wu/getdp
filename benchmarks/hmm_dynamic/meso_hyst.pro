...
Function {
  ...
  //EndIf
  //If (!CML)
  a_pert_previous[] = (currentTimeStep == 1) ? Vector[0.,0.,0.] : Vector[ 0., 0., ScalarField[XYZ[]]{0}] ;
  a_tot_previous[]  = (currentTimeStep == 1) ? Vector[0.,0.,0.] : Vector[ 0., 0., ScalarField[XYZ[]]{1}] ;
  h_tot_previous[]  = (currentTimeStep == 1) ? Vector[0.,0.,0.] : VectorField[XYZ[]]{2} ;
  b_tot_previous[]  = (currentTimeStep == 1) ? Vector[0.,0.,0.] : VectorField[XYZ[]]{3} ;
  //EndIf
  ...
  //Gyselink's paper: Jiles-Atherton vector 2D model
  //================================================
  Msat             = 1145220;
  aaa              = 59.5;
  kkk              = 99.2;
...
FunctionSpace{
  ...
  { Name BLosses_t0~{iP} ; Type Vector;
    BasisFunction {
      { Name sex ; NameOfCoef aex ; Function BF_VolumeX ; Support Omega ; Entity VolumesOf[ Omega ] ; }
      { Name sey ; NameOfCoef aey ; Function BF_VolumeY ; Support Omega ; Entity VolumesOf[ Omega ] ; }
      { Name sez ; NameOfCoef aez ; Function BF_VolumeZ ; Support Omega ; Entity VolumesOf[ Omega ] ; }
...
Formulation {
  ...
  { Name Init_PreviousTimeStep~{iP} ; Type FemEquation ;
    Quantity{
      ...
      { Name b  ; Type Local  ; NameOfSpace BLosses_t0~{iP} ; }
    }
    Equation{
      ...
      Galerkin { [     Dof{b}       , {b} ]; In Omega; Jacobian Vol; Integration II; }
      Galerkin { [ -b_tot_previous[], {b} ]; In Omega; Jacobian Vol; Integration II; }
    }
  }
  ...
  { Name a_NR~{iP} ; Type FemEquation ;
    Quantity {
      ...
      { Name b0     ; Type Local  ; NameOfSpace BLosses_t0~{iP} ; }
    }
    Equation {
      ...
      Galerkin { [ nu[ {d a} + bM[] + Pert~{iP}[] ] * Dof{d a} , {d a} ]   ;   In Omega_L; Jacobian Vol; Integration II; }
      Galerkin { [ nu[ {d a} + bM[] + Pert~{iP}[] ] * bM[], {d a} ]      ;   In Omega_L; Jacobian Vol; Integration II; }
      Galerkin { [ nu[ {d a} + bM[] + Pert~{iP}[] ] * Pert~{iP}[] , {d a} ];   In Omega_L; Jacobian Vol; Integration II; }

      Galerkin { [ h_Jiles[{h0}, {b0}, {d a} + bM[]+Pert~{iP}[] ]{List[hyst_FeSi]}#4 , {d a} ];
        In Omega_NL; Jacobian Vol; Integration II; }
      Galerkin { JacNL[ dhdb_Jiles[ {h}, {d a} + bM[]+Pert~{iP}[], {h}-{h0} ]{List[hyst_FeSi]} * Dof{d a} , {d a} ];
        In Omega_NL ; Jacobian Vol ; Integration II ; }
      Galerkin { JacNL[ dhdb_Jiles[ {h}, {d a} + bM[]+Pert~{iP}[], {h}-{h0} ]{List[hyst_FeSi]} * bM[] , {d a} ];
        In Omega_NL ; Jacobian Vol ; Integration II ; }
      Galerkin { JacNL[ dhdb_Jiles[ {h}, {d a} + bM[]+Pert~{iP}[], {h}-{h0} ]{List[hyst_FeSi]} * Pert~{iP}[], {d a} ];
        In Omega_NL ; Jacobian Vol ; Integration II ; }
...
Resolution {
  { Name a_Init ;
    ...
    Operation {
      ...
      //GmshRead[StrCat(Dir_Meso_Comp, Sprintf("a_tot_Prob1_TS%g_Elenum%g.pos", (currentTimeStep-1), ELENUM) ) ];
      //  GmshRead[StrCat(Dir_Meso_Comp, Sprintf("h_tot_Prob1_TS%g_Elenum%g.pos", (currentTimeStep-1), ELENUM) ) ];
      //  GmshRead[StrCat(Dir_Meso_Comp, Sprintf("b_tot_Prob1_TS%g_Elenum%g.pos", (currentTimeStep-1), ELENUM) ) ];
        Generate[AHHH~{iP}]; Solve[AHHH~{iP}]; TransferSolution[AHHH~{iP}];
      EndIf
      EndFor
    }
...
PostProcessing {
  ...
  { Name a_Micro_NR~{iP} ; NameOfFormulation a_NR~{iP} ; NameOfSystem Micro~{iP} ;
     PostQuantity {
       ...
       { Name h_tot ; Value {
           Term { [ nu[] * ({d a} + bM[] + Pert~{iP}[]) ]; In Omega_L; Jacobian Vol;}
           Term { [ {h} ]; In Omega_NL; Jacobian Vol;} } }
       { Name h_tot_mean ; Value {// stored in #22
           Integral { [ nu[] * ({d a} + bM[] + Pert~{iP}[])/#12 ] ; In Omega_L ; Jacobian Vol; Integration II ; }
       ...
       { Name MagEnergy_tot_mean  ; Value{ // stored in #27. Contribution of the current time step to the integral \oint (h db)
           Integral { [ ( nu[] * ({d a} + bM[] + Pert~{iP}[] ) * (Dt[{d a}] + dt_bM[]) )/#12 ] ;
             In Omega_L ; Jacobian Vol; Integration II ; }
