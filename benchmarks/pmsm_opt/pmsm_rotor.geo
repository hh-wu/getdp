...
//--
If(Flag_IPM)
  lin[] = Extrude {{0, 0, 1}, {0, 0, 0}, AA[1]} { Point{pMagnet[2]}; };
  pMagnet[] += lin[0]; cirR[]+=lin[1]; 
EndIf
//--
Printf("pMagnet[]:", pMagnet[]);
Printf("cirR1[]:", cirR[]);
...
If(!Flag_IPM)
  ...
  Line Loop(newll) = {linRotor[]};
  srotor[0]=news; Plane Surface(srotor[0]) = {newll-1};
  Printf("linRotor[]:", linRotor[]);
Else
  linRotor[] = {linR0[{0,1}], cirR[4], -linR2[0], cirR[3], linR3[0],
               cirR[6], -linR1[{1,0}], -cirR[{2,1,0}]};
  Line Loop(newll) = {linRotor[]};
  srotor[0]=news; Plane Surface(srotor[0]) = {newll-1};
  Printf("linRotor[]:", linRotor[]);
  linRotor2[] = {linR0[2], cirR[{7,8,9}],-linR1[2],-cirR[{6,5,4}]};
  Line Loop(newll) = {linRotor2[]};
  Printf("linRotor2[]:", linRotor2[]);
  srotor[1]=news; Plane Surface(srotor[1]) = {newll-1};
EndIf

If(!Flag_IPM)
  lMagnet[] = { linR2[1], cirR[7], -linR3[1], -linR3[0], -cirR[3], linR2[0] };
Else
  lMagnet[] = { cirR[5], -linR3[0], -cirR[3], linR2[0] };
EndIf
Line Loop(newl) = {lMagnet[]};
...
Else
  Physical Surface(ROTOR_FE) = {srotor[0]};
  Physical Surface(ROTOR_FE_PM) = {srotor[1]};
  Physical Surface(ROTOR_AIR) = {};  
EndIf
Physical Surface(ROTOR_AIRGAP) = {sairrotormb[]};

For k In {0:NbrPolesInModel - 1}
  Physical Surface(ROTOR_MAGNET+k) = {smagnet[k]}; 
EndFor

Physical Line(SURF_INT) = {surfint[]}; 
