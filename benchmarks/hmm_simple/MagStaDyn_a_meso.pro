  ...
  For iP In {1:Nbr_SubProblems}
    ...
    { Name a_NR~{iP} ; Type FemEquation ;
      ...
      Equation {
        ...
        Galerkin { JacNL[ dhdb[{d a}+bM[]+Pert~{iP}[] ] * Dof{d a}, {d a} ]; In Omega_NL; Jacobian Vol; Integration II; }
        ...
        If(Flag_Dynamic)
          ...
          Galerkin { [   sigma[] * ( factor * dt_bM[] /\ XYZ[] ) , {a} ]; In Omega_C; Jacobian Vol; Integration II; }
          ...
          Galerkin { [   sigma[] * ( factor * dt_bM[] /\ XYZ[] ) , {ur} ]; In Omega_C; Jacobian Vol; Integration II; }
  ...
  { Name a_Init ; Hidden 1;
    ...
    Operation {
      For iP In {1:Nbr_SubProblems}
        ...
        EndIf
        If(TSCURRENT != 1)
          GmshRead[StrCat(Dir_Meso, Sprintf("a_pert_Prob1_TS%g_Elenum%g.pos", (TSCURRENT - 1), ELENUM) ) ];
          Generate[AH~{iP}]; Solve[AH~{iP}]; TransferSolution[AH~{iP}];
        EndIf
      EndFor
    }
  ...
  For iP In {1:Nbr_SubProblems}
    { Name a_Micro_NR~{iP} ; NameOfFormulation a_NR~{iP} ; NameOfSystem Micro~{iP} ;
      PostQuantity {
        ...
        { Name b_mean ; Value {
            Integral { [ ({d a} + bM[] + Pert~{iP}[])/#12 ] ; In Omega ; Jacobian Vol ; Integration II ; } } }
        { Name h ; Value {
        ...
        If(!Flag_Dynamic)
          ...
          { Name MagneticEnergy_mean  ; Value{ // stored in #27. Contribution of the current time step to the integral \oint (h db)
              Integral { [ ( nu[ {d a} + bM[] + Pert~{iP}[]] * ({d a} + bM[] + Pert~{iP}[] ) * ({d a} + bM[]) )/#12 ] ;
        ...
        If(Flag_Dynamic)
          ...
          { Name e_proj ; Value { Term { [ factor * ( XYZ[] /\ dt_bM[] ) ] ; In Omega_C ; Jacobian Vol;  } } }
          { Name e ; Value {
              Term { [ - Dt[{a}] ]   ; In Omega_C ; Jacobian Vol;  }
              Term { [ - {ur} ]      ; In Omega_C ; Jacobian Vol;  }
          ...
          { Name j_proj ; Value { Term { [ factor * sigma[] * ( XYZ[] /\ dt_bM[] ) ] ; In Omega_C ; Jacobian Vol;  } } }
          { Name j ; Value {
              Term { [ - sigma[] * Dt[{a}] ] ; In Omega_C ; Jacobian Vol;  }
              Term { [ - sigma[] * {ur} ] ; In Omega_C ; Jacobian Vol;  }
              Term { [   factor * sigma[] * ( XYZ[] /\ dt_bM[] ) ] ; In Omega_C ; Jacobian Vol;  } } }

          { Name JouleLosses_mean  ; Value{ // stored in #25
              Integral { [ ( sigma[] * SquNorm[ Dt[{a}] + {ur} - factor * ( XYZ[] /\ dt_bM[] ) ] )/#12 ] ;
                In Omega_C ; Jacobian Vol; Integration II ; } } }
          { Name MagneticEnergy_mean  ; Value{ // stored in #27. Contribution of the current time step to the integral \oint (h db)
...
PostOperation {
  For iP In {1:Nbr_SubProblems}
    { Name mean~{iP} ; NameOfPostProcessing a_Micro_NR~{iP}; LastTimeStepOnly; Format Table;
      Operation{
        Print[ vol[Omega]       , OnGlobal, Store 12, File StrCat[Dir_Meso, "vol.txt"] ] ;
        Print[ h_mean[Omega], OnGlobal, Store 22, File StrCat[Dir_Meso, Sprintf("h%g_%g.txt", iP, ELENUM) ]] ;
        Print[ b_mean[Omega], OnGlobal, Store 21, File StrCat[Dir_Meso, Sprintf("b%g_%g.txt", iP, ELENUM) ]] ;
        If(iP == 1)
          Print[ JouleLosses_mean[Omega_C], OnGlobal, Store 25, File StrCat[Dir_Meso, Sprintf("JouleLosses_%g.txt", ELENUM) ]] ;
          Print[ MagneticEnergy_mean[Omega], OnGlobal, Store 28, File StrCat[Dir_Meso, Sprintf("MagneticEnergy_%g.txt", ELENUM) ] ] ;
        EndIf
      }
    }
    { Name map_field~{iP} ; NameOfPostProcessing a_Micro_NR~{iP}; LastTimeStepOnly; Format Gmsh;
      Operation {
        If(!Flag_Dynamic)
          Print[ a_pert, OnElementsOf Omega, File StrCat[Dir_Meso, Sprintf("a_pert_Prob%g_Elenum%g.pos" , iP, ELENUM) ] ];
          Print[ a_proj, OnElementsOf Omega, File StrCat[Dir_Meso, Sprintf("a_proj_Prob%g_Elenum%g.pos" , iP, ELENUM) ] ];
          Print[ a , OnElementsOf Omega, File StrCat[Dir_Meso, Sprintf("a_Prob%g_Elenum%g.pos"  , iP, ELENUM) ] ];
          Print[ b_pert, OnElementsOf Omega, File StrCat[Dir_Meso, Sprintf("b_pert_Prob%g_Elenum%g.pos" , iP, ELENUM) ] ];
          Print[ b , OnElementsOf Omega, File StrCat[Dir_Meso, Sprintf("b_Prob%g_Elenum%g.pos"  , iP, ELENUM) ] ];
          Print[ b_proj, OnElementsOf Omega, File StrCat[Dir_Meso, Sprintf("b_proj_Prob%g_Elenum%g.pos" , iP, ELENUM) ] ];
          Print[ h , OnElementsOf Omega, File StrCat[Dir_Meso, Sprintf("h_Prob%g_Elenum%g.pos"  , iP, ELENUM) ] ];
        EndIf
        If(Flag_Dynamic)
          Print[ a_pert,  OnElementsOf Omega, File StrCat[Dir_Meso, Sprintf("a_pert_Prob%g_Elenum%g.pos" , iP, ELENUM) ],
          ...
          Print[ e_pert,  OnElementsOf Omega, File StrCat[Dir_Meso, Sprintf("e_pert_Prob%g_Elenum%g.pos" , iP, ELENUM) ],
            AppendToExistingFile (TSCURRENT > 1 ? 1 : 0), NoMesh (TSCURRENT > 1 ? 1 : 0), OverrideTimeStepValue TSCURRENT ];
          Print[ e_proj,  OnElementsOf Omega, File StrCat[Dir_Meso, Sprintf("e_proj_Prob%g_Elenum%g.pos" , iP, ELENUM) ],
            AppendToExistingFile (TSCURRENT > 1 ? 1 : 0), NoMesh (TSCURRENT > 1 ? 1 : 0), OverrideTimeStepValue TSCURRENT ];
          Print[ e,  OnElementsOf Omega, File StrCat[Dir_Meso, Sprintf("e_Prob%g_Elenum%g.pos" , iP, ELENUM) ],
            AppendToExistingFile (TSCURRENT > 1 ? 1 : 0), NoMesh (TSCURRENT > 1 ? 1 : 0), OverrideTimeStepValue TSCURRENT ];

          Print[ j_pert,  OnElementsOf Omega, File StrCat[Dir_Meso, Sprintf("j_pert_Prob%g_Elenum%g.pos" , iP, ELENUM) ],
            AppendToExistingFile (TSCURRENT > 1 ? 1 : 0), NoMesh (TSCURRENT > 1 ? 1 : 0), OverrideTimeStepValue TSCURRENT ];
          Print[ j_proj,  OnElementsOf Omega, File StrCat[Dir_Meso, Sprintf("j_proj_Prob%g_Elenum%g.pos" , iP, ELENUM) ],
            AppendToExistingFile (TSCURRENT > 1 ? 1 : 0), NoMesh (TSCURRENT > 1 ? 1 : 0), OverrideTimeStepValue TSCURRENT ];
          Print[ j,  OnElementsOf Omega, File StrCat[Dir_Meso, Sprintf("j_Prob%g_Elenum%g.pos" , iP, ELENUM) ],
            AppendToExistingFile (TSCURRENT > 1 ? 1 : 0), NoMesh (TSCURRENT > 1 ? 1 : 0), OverrideTimeStepValue TSCURRENT ];
    ...
    { Name init_field~{iP} ; NameOfPostProcessing a_Micro_NR~{iP};
      Operation {
        Print[ a_pert, OnElementsOf Omega, File StrCat[Dir_Meso, Sprintf("a_pert_Prob%g_TS%g_Elenum%g.pos", iP, TSCURRENT, ELENUM) ],
