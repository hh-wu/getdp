...
DefineConstant[
  ...
  //lc1 = {8*mm, Name "Geo/Mesh Characteristic Length Factor",Visible 1},
  lc2 = {20*mm, Name "Parameters/2Mesh size at infinity [m]",Visible 1},
//  lc1 = {TotalMemory <= 1024 ? 10*mm : 8*mm*lchar, Name "Parameters/2Mesh size on magnets [m]"}
//  lc2 = {TotalMemory <= 1024 ? 40*mm : 20*mm*lchar, Name "Parameters/2Mesh size at infinity [m]"}
...
For i In {1:NumMagnets}
  If(M~{i} == 0) // cylinder
    ...
    pMagnet~{i}[] = {p1,p2,p3,p4};
    Printf("pMagnet~{%g}[] = %g,%g,%g,%g",i,pMagnet~{i}[]);
    l1 = newl; Line(l1) = {p1,p2}; l2 = newl; Line(l2) = {p2,p4};
    l3 = newl; Line(l3) = {p4,p3}; l4 = newl; Line(l4) = {p3,p1};
    ll1 = newll; Line Loop(ll1) = {l1,l2,l3,l4};
    lMagnet~{i}[] = {l1,l2,l3,l4};
    Printf("lMagnet~{%g}[] = %g,%g,%g,%g",i,lMagnet~{i}[]);
    ...
    Printf("Magnet~{%g}[] = %g,%g,%g,%g",i,Magnet~{i}[]);    
  EndIf
