...
// Normals are all pointing outside subdomains and full domain (normal vector is pointing inside the obstacle)

Include "circle_pie_data.geo";

/*If(NON_CONFORM)
  Include "NC_circle_pie.geo";
EndIf*/
...
//If(!NON_CONFORM)
  ...
  For i In {0:N_DOM}
    ...
    l_GammaInf[] = {};
    If(i != N_DOM)
      p_scat[i] = newp; Point(p_scat[i]) = {R_INT * Cos(t_dn+angleRotate), R_INT * Sin(t_dn+angleRotate),0,LC};
      p_inf[i] = newp; Point(p_inf[i]) = {R_EXT * Cos(t_dn+angleRotate), R_EXT * Sin(t_dn+angleRotate),0,LC};
      l_sigma[i] = newl; Line(l_sigma[i]) = {p_inf[i], p_scat[i]}; //from infinity to the scatterer
      If(N_DOM == 2)
	t_up = (i+1) * 2 * Pi / N_DOM;
	t_half = 0.5*(t_up + t_dn);
	p_scatbis[i] = newp; Point(p_scatbis[i]) = {R_INT * Cos(t_half+angleRotate), R_INT * Sin(t_half+angleRotate),0,LC};
	p_infbis[i] = newp; Point(p_infbis[i]) = {R_EXT * Cos(t_half+angleRotate), R_EXT * Sin(t_half+angleRotate),0,LC};
      EndIf
    EndIf
    If(idom >= 0)
      If(N_DOM == 2)
	...
	l_GammaInf[0] = newl; Circle(l_GammaInf[0]) = {p_inf[i-1], 1, p_inf[i%N_DOM]};
      ...
      Physical Line(3000 + idom) = {-l_sigma[i-1]}; // "Left" transm. boundary (if looking to the center from infinity)
