// THE FOLLOWING CODE SOLVES A STRONGLY COUPLED (AKA. MONOLITHIC) ELECTROELASTIC FORMULATION ON THE UNDEFORMED OR DEFORMED MECHANIC DOMAIN (IN THE LATTER CASE THE MECHANIC COMPUTATION SHOULD BE SOMEWHAT MORE ACCURATE SINCE THE STIFFNESS OF THE MEMBRANE IS COMPUTED ON THE DEFORMED MEMBRANE).
// THE CODE IS NOT (YET) MEANT TO BE USED FOR DDM WITH MULTIPLE SUBDOMAINS SO PLEASE LEAVE NX AND NY TO 1. THE CODE IS JUST THERE TO SHOW THE STRONG COUPLED ELECTROELASTIC IMPLEMENTATION USING NEWTON ITERATIONS IN GETDP. A FURTHER VERSION WILL ENABLE DDM COMPUTATIONS 
...
Formulation	{
	For x In {0:nx-1}
		For y In {0:ny-1}		
			{ Name monolithic_formulation~{x+nx*y+1}; Type FemEquation; Quantity{
				...
				Equation{
					 ...
					 In Region[solid~{x+nx*y+1}] ; Jacobian JVol ; Integration I1 ; }
				       ...
				       // Kvu part of the Jacobian
				       Galerkin { JacNL[ -epsilon[]* dFxdv[Dof{u_x}*0 + {d v}], {d v} ] ;
					 In electric_domain~{x+nx*y+1} ; Jacobian JVol ; Integration I1;}
				       Galerkin { JacNL[ -epsilon[]* dFydv[Dof{u_y}*0 + {d v}], {d v} ] ;
					 In electric_domain~{x+nx*y+1} ; Jacobian JVol ; Integration I1;}
				       Galerkin { JacNL[ -epsilon[]* dFzdv[Dof{u_z}*0 + {d v}], {d v} ] ;
					 In electric_domain~{x+nx*y+1} ; Jacobian JVol ; Integration I1;}

				       // Kuu
				       Galerkin { JacNL[ epsilon[]* dFxdux[Dof{u_x}*0 + {d v}], {D3 u_x} ] ;
					 In electric_domain~{x+nx*y+1} ; Jacobian JVol ; Integration I1;}
				       Galerkin { JacNL[ epsilon[]* dFydux[Dof{u_y}*0 + {d v}], {D3 u_x} ] ;
					 In electric_domain~{x+nx*y+1} ; Jacobian JVol ; Integration I1;}
				       Galerkin { JacNL[ epsilon[]* dFzdux[Dof{u_z}*0 + {d v}], {D3 u_x} ] ;
					 In electric_domain~{x+nx*y+1} ; Jacobian JVol ; Integration I1;}

				       Galerkin { JacNL[ epsilon[]* dFxduy[Dof{u_x}*0 + {d v}], {D3 u_y} ] ;
					 In electric_domain~{x+nx*y+1} ; Jacobian JVol ; Integration I1;}
				       Galerkin { JacNL[ epsilon[]* dFyduy[Dof{u_y}*0 + {d v}], {D3 u_y} ] ;
					 In electric_domain~{x+nx*y+1} ; Jacobian JVol ; Integration I1;}
				       Galerkin { JacNL[ epsilon[]* dFzduy[Dof{u_z}*0 + {d v}], {D3 u_y} ] ;
					 In electric_domain~{x+nx*y+1} ; Jacobian JVol ; Integration I1;}

				       Galerkin { JacNL[ epsilon[]* dFxduz[Dof{u_x}*0 + {d v}], {D3 u_z} ] ;
					 In electric_domain~{x+nx*y+1} ; Jacobian JVol ; Integration I1;}
				       Galerkin { JacNL[ epsilon[]* dFyduz[Dof{u_y}*0 + {d v}], {D3 u_z} ] ;
					 In electric_domain~{x+nx*y+1} ; Jacobian JVol ; Integration I1;}
				       Galerkin { JacNL[ epsilon[]* dFzduz[Dof{u_z}*0 + {d v}], {D3 u_z} ] ;
					 In electric_domain~{x+nx*y+1} ; Jacobian JVol ; Integration I1;}

				       // Kuv
				       Galerkin { JacNL[ -epsilon[]* dWedxdv[Dof{d v}*0 + {d v}], {D3 u_x} ] ;
					 In electric_domain~{x+nx*y+1} ; Jacobian JVol ; Integration I1;}
				       Galerkin { JacNL[ -epsilon[]* dWedydv[Dof{d v}*0 + {d v}], {D3 u_y} ] ;
					 In electric_domain~{x+nx*y+1} ; Jacobian JVol ; Integration I1;}
				       Galerkin { JacNL[ -epsilon[]* dWedzdv[Dof{d v}*0 + {d v}], {D3 u_z} ] ;
					 In electric_domain~{x+nx*y+1} ; Jacobian JVol ; Integration I1;}
...
Resolution	{
		{Name res; 
			System	{
				For x In {0:nx-1}
					For y In {0:ny-1}
						{Name monolithic_res~{x+nx*y+1}; NameOfFormulation monolithic_formulation~{x+nx*y+1}; Type Complex; Frequency freq;}
			...
			Operation{
				...
				For x In {0:nx-1}
					For y In {0:ny-1}
						...
						InitSolution[monolithic_res~{x+nx*y+1}];
				...
				UpdateConstraint[monolithic_res~{x+nx*y+1}, Region[{sigma_up~{x+nx*y+1}, 
				...
				Operation {	
					...
					// This is for the mechanic computation on the undeformed mesh:
      						GenerateJacGroupCumulative[monolithic_res~{x+nx*y+1}, electric_domain~{x+nx*y+1}];



					// This is for the mechanic computation on the deformed mesh:
      						//GenerateJac[monolithic_res~{x+nx*y+1}];
					...
					// We now want to store the final deformation on the undeformed mesh:
				...
				///// END NON LINEAR ITERATIVE LOOP
						...
						SaveSolution[mesh_deform~{x+nx*y+1}]; 
						SaveSolution[monolithic_res~{x+nx*y+1}];
...
PostProcessing 	{
		For x In {0:nx-1}
			For y In {0:ny-1}
  				...
  				{ Name u~{x+nx*y+1}; NameOfFormulation monolithic_formulation~{x+nx*y+1};
    					Quantity {
  				...
  				{ Name v~{x+nx*y+1}; NameOfFormulation monolithic_formulation~{x+nx*y+1};
    					Quantity {
				...
				{ Name init_fields_mechanic~{x+nx*y+1}; NameOfFormulation monolithic_formulation~{x+nx*y+1};
					Quantity {
				...
				{ Name init_fields_electric~{x+nx*y+1}; NameOfFormulation monolithic_formulation~{x+nx*y+1};
					Quantity {
				      		{ Name init ; Value { Local { [ 0 ]; 
							In Region[{sigma_up~{x+nx*y+1}, sigma_right~{x+nx*y+1}, 
								sigma_down~{x+nx*y+1}, sigma_left~{x+nx*y+1}}] ; Jacobian JSur ; } } }
