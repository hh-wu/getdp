...
Formulation {
  ...
  { Name MagStaDyn_a_2D ; Type FemEquation ;
    ...
    Equation {
      Galerkin { [ nxh[], {a} ];    In Boundary ; Jacobian Vol ; Integration I1 ; }
...
PostOperation Get_AllTS UsingPost MagStaDyn_a_2D {
  ...
  If(Flag_NL_law == NL_JA )
    For k In {1:num_postop_points}
      Print[ hb, OnPoint {xpos~{k},ypos~{k},0}, Format TimeTable, File StrCat[Dir,Sprintf("hbp_%g_all",k),ExtGnuplot] ];
    EndFor
  EndIf
...
PostOperation {
  ...
  { Name a_only ; NameOfPostProcessing MagStaDyn_a_2D ;
    Operation {
