...
PostProcessing {
  ...
  // (only for 'a' and js0[] related quantities) (for 'j', DomainC_Mag must be empty)
	...
	Term { [ -nu[] * br[] ] ; In DomainMagnet_Mag ; Jacobian Vol ; } } }
      ...
      { Name ReU ; Value { Term { [ Re[{U}] ]   ; In DomainC_Mag ; }
                           Term { [ Re[{Us}] ]  ; In DomainS_Mag ; }
                           Term { [ Re[{Uz}] ]  ; In DomainZt_Cir ; } } }
      { Name ImU ; Value { Term { [ Im[{U}] ]   ; In DomainC_Mag ; }
                           Term { [ Im[{Us}] ]  ; In DomainS_Mag ; }
                           Term { [ Im[{Uz}] ]  ; In DomainZt_Cir ; } } }

      { Name ReI ; Value { Term { [ Re[{I}] ]   ; In DomainC_Mag ; }
                           Term { [ Re[{Is}] ]  ; In DomainS_Mag ; }
                           Term { [ Re[{Iz}] ]  ; In DomainZt_Cir ; } } }
      { Name ImI ; Value { Term { [ Im[{I}] ]   ; In DomainC_Mag ; }
                           Term { [ Im[{Is}] ]  ; In DomainS_Mag ; }
...
Group { // Whole thermal domain with convection surfaces
  DomainTot_The = Region[ {Domain_The, SurfaceConv_The} ] ;
}
  ...
  { Name Hgrad_T; Type Form0 ;
    BasisFunction {
      { Name sn ; NameOfCoef Tn ; Function BF_Node ;
        Support DomainTot_The ; Entity NodesOf[All] ; }
    }
    Constraint {
  ...
  { Name Thermal_T ; Type FemEquation ;
    ...
    Equation {
      Galerkin { [ lambda[{T}] * Dof{d T} , {d T} ] ;
                 In Domain_The; Integration Gauss_dv_dv ; Jacobian Vol ; }

      Galerkin { DtDof[ rhoc[{T}]  * Dof{T} , {T} ] ;
                 In Domain_The; Integration Gauss_v_v ; Jacobian Vol ; }

      //Galerkin { [ -0.5 * sigma[{T}] * <a>[SquNorm[Dt[{a}]]] , {T} ] ;
      //          In DomainJoule_The ; Integration Gauss_v_v ; Jacobian Vol ; }

      Galerkin { [ - CoefPower * sigma[{T}] * <a>[SquNorm[Dt[{a}]+{ur}/CoefGeos[]]] ] ;
                 In DomainC_Mag ; Integration Gauss_v_v; Jacobian Vol ; }

      Galerkin { [ - CoefPower * 1./sigma[{T}] * <ir>[SquNorm[(js0[]*Vector[0,0,1])*{ir}]] ] ;
                 In DomainS_Mag ; Integration Gauss_v_v; Jacobian Vol ; }

      Galerkin { [ h[] * Dof{T} , {T} ] ;
                 In SurfaceConv_The ; Integration Gauss_v_v; Jacobian Sur ; }
...
Resolution {
  { Name MagDynThe_av_2D ;
    ...
    Operation {
      InitSolution[Sys_The] ; SaveSolution[Sys_The] ;
      Generate[Sys_Mag]; Solve[Sys_Mag]; SaveSolution[Sys_Mag];
      TimeLoopTheta[Time0, Time1, DTime, 1.]{
...
PostProcessing {
  { Name Thermal_T ; NameOfFormulation Thermal_T ;
    PostQuantity {
      { Name T ; Value { Term { [ {T} ] ; In Domain_The ; } } }
    }
  }
}
*/
