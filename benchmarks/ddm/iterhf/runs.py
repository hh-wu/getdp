...
outputFilenameGetdp = 'getdpOut_Nv%d_Nh%d_k%.2f_solverFem_nLambda%d_ref.txt'%(Nv, Nh, k, nlambda)
...
for lcRatio in lcRatioList:
    ...
    outputFilenameGmsh = 'error_Nv%d_Nh%d_k%.2f_lcRatio%.1f_solverFem_nLambda%d.txt'%(Nv, Nh, k, lcRatio, nlambda)
    outputFilenameGetdp = 'getdpOut_Nv%d_Nh%d_k%.2f_lcRatio%.1f_solverFem_nLambda%d.txt'%(Nv, Nh, k, lcRatio, nlambda)
    ...
    system('gmsh combine_from_u.geo -bin -v 3 - > %s'%outputFilenameGmsh)
