...
Constraint {
  ...
  { Name Dirichlet_e_vol;
    Case { { Region Omega ; Type Assign ;  } } 
    // Case { { Region GammaScat ; Type Assign ;  } } 
    // Case { { Region GammaInf ; Type Assign ;  } } 
  }
}

FunctionSpace {
  { Name Hcurl_e_inc; Type Form1;
    BasisFunction {
      { Name se; NameOfCoef ee; Function BF_Edge; 
	Support Region[{Omega/*,GammaScat,GammaInf*/}] ; Entity EdgesOf[All]; }
    }
    Constraint {
      { NameOfCoef ee; EntityType EdgesOf ; NameOfConstraint Dirichlet_e_vol; }
    }
  }
...
Formulation {
  { Name Maxwell_e_inc; // For imposing the source on edges
    Quantity { 
      { Name e; Type Local; NameOfSpace Hcurl_e_inc; }
    }
    Equation {
      Galerkin { [ Dof{e} , {e} ]; 
	In Region[ {Omega}]; Integration I1; Jacobian JVol;  }
      Galerkin { [ Vector[0,0,0] , {e} ]; 
      	In Region[ {Omega}]; Integration I1; Jacobian JVol;  }
    }
  }
  ...
  { Name Maxwell_Lagrange; Type FemEquation; 
	...
	In Omega; Integration I1; Jacobian JVol;  }
      // Galerkin { [ -I[] * k * ( N[] /\ Dof{e_lag} ) /\ N[] , {e_lag} ];
	Galerkin { [ I[] * k * (N[]) /\ ( N[] /\ Dof{e_lag} ) , {e_lag} ];
	In GammaInf; Integration I1; Jacobian JSur;  }
      //boundary condition
...
Resolution {
  { Name Maxwell_e_inc ;
    System {
      { Name A ; NameOfFormulation Maxwell_e_inc ; Type Complex; }
    }
    Operation { 
      Generate[A] ; Solve[A] ;
    }
...
PostProcessing {
  { Name Maxwell_e_inc ; NameOfFormulation Maxwell_e_inc ;
    Quantity {
  ...
  {Name Maxwell_Lagrange ; NameOfFormulation Maxwell_Lagrange ;
    Quantity{
      ...
      { Name e_inc ; Value { Local { [ einc[] ] ; In GammaScat; Jacobian JSur ; } } }
...
PostOperation {
  { Name Maxwell_e_inc ; NameOfPostProcessing Maxwell_e_inc; 
    Operation {
      Print[ e_vol, OnElementsOf Omega, File "e_inc_vol.pos"] ;
    }
  }
  ...
  { Name Maxwell_Lagrange ; NameOfPostProcessing Maxwell_Lagrange; 
    Operation {
      ...
      Print[ e_inc, OnElementsOf GammaScat, File "e_inc.pos"] ;
