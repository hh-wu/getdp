...
Group{ // definition of some subsets of the regions defined by the user
  For ii In {0: #ListOfDom()-1}
    ...
    EndFor
      TrGr~{idom} = ElementsOf[Omega~{idom}, OnOneSideOf Gama_D~{idom}];
  EndFor
}
...
FunctionSpace {
  For ii In {0: #ListOfDom()-1}
    ...
    /////////////////////////////////////////////////////////////////
  { Name Hgrad_u_pml_primal~{idom}~{jdom} ; Type Form0 ;
    BasisFunction {
      { Name sn ; NameOfCoef un ; Function BF_Node ;
	Support Region[{OmegaPml~{idom}~{jdom}, Sigma~{idom}~{jdom}, GamaPml_S~{idom}~{jdom}}]; Entity NodesOf[All];
      }
    }
    Constraint {
      { NameOfCoef un ; EntityType NodesOf ; NameOfConstraint Dirichlet0~{idom} ; }
  ...
  { Name Hgrad_u_pml_dual~{idom}~{jdom} ; Type Form0 ;
    BasisFunction {
      { Name sn ; NameOfCoef un ; Function BF_Node ;
	Support Region[{OmegaPml~{idom}~{jdom}, Sigma~{idom}~{jdom}, GamaPml_S~{idom}~{jdom}}]; Entity NodesOf[All];
      }
  ...
  { Name Hgrad_gin~{idom}~{jdom} ; Type Form0 ;
    BasisFunction {
      { Name sn ; NameOfCoef un ; Function BF_Node ;
  ...
  { Name Hgrad_gout~{idom}~{jdom} ; Type Form0 ;
    BasisFunction {
      { Name sn ; NameOfCoef un ; Function BF_Node ;
	Support Region[{Sigma~{idom}~{jdom}}]; Entity NodesOf[All];
      }
  ...
  { Name Hgrad_lambda~{idom}~{jdom} ; Type Form0 ;
    BasisFunction {
      { Name sn ; NameOfCoef un ; Function BF_Node ;
  	Support Region[{Sigma~{idom}~{jdom}}]; Entity NodesOf[All];
      }
...
Formulation {
  ...
  { Name DDM~{idom} ; Type FemEquation ;
    Quantity {
      ...
      { Name gin~{idom}~{0} ; Type Local ; NameOfSpace Hgrad_gin~{idom}~{0} ;}
      { Name gin~{idom}~{1} ; Type Local ; NameOfSpace Hgrad_gin~{idom}~{1} ;}
    }
    Equation {
	...
	// g_in LEFT (#10 > 0) or 0 (#10 == 0)
	Galerkin { [ (#10 > 0. ? 2.*g_in~{idom}~{0}[] : 0), {u~{idom}} ] ;
	  In Sigma~{idom}~{0}; Jacobian JSur ; Integration I1 ; }
	// g_in RIGHT (#11 > 0) or 0 (#11 == 0)
	Galerkin { [ (#11 > 0. ? 2.*g_in~{idom}~{1}[] : 0), {u~{idom}} ] ;
	  In Sigma~{idom}~{1}; Jacobian JSur ; Integration I1 ; }

	If (dPML)
	// corrections
	Galerkin { [ (#17 > 0 ? -{gin~{idom}~{0}} : 0.), {u~{idom}} ] ;
	  In Sigma~{idom}~{0}; Jacobian JSur ; Integration I1 ; }
	Galerkin { [ (#17 > 0 ? -{gin~{idom}~{1}} : 0.), {u~{idom}} ] ;
	  In Sigma~{idom}~{1}; Jacobian JSur ; Integration I1 ; }
	EndIf
  ...
  { Name WaveDirichlet2D_pml_primal~{idom}~{jdom} ; Type FemEquation ;
    Quantity {
      { Name uD ; Type Local ; NameOfSpace Hgrad_u_Dirichlet2D~{idom} ;}
      { Name u ; Type Local ; NameOfSpace Hgrad_u_pml_primal~{idom}~{jdom} ;}
      { Name lambda ; Type Local ; NameOfSpace Hgrad_lambda~{idom}~{jdom} ;}
    }
    Equation {
      //modified Helmholtz equation
      Galerkin{[D[]* Dof{Grad u}, {Grad u}];
      	In OmegaPml~{idom}~{jdom}; Jacobian JVol; Integration I1;}
      Galerkin{[-kPml~{idom}~{jdom}[]^2*Kx[]*Ky[]*Dof{u}, {u}];
      	In OmegaPml~{idom}~{jdom}; Jacobian JVol; Integration I1;}
  ...
  { Name WaveDirichlet2D_pml_dual~{idom}~{jdom} ; Type FemEquation ;
    Quantity {
      { Name u ; Type Local ; NameOfSpace Hgrad_u_pml_dual~{idom}~{jdom} ;}
      { Name gout ; Type Local ; NameOfSpace Hgrad_gout~{idom}~{jdom} ;}
    }
    Equation {
      //modified Helmholtz equation
      Galerkin{[D[]* Dof{Grad u}, {Grad u}];
  ...
  { Name Compute_gout~{idom}~{jdom} ; Type FemEquation ;
    Quantity {
      { Name gout ; Type Local ; NameOfSpace Hgrad_gout~{idom}~{jdom} ;}
      { Name gin ; Type Local ; NameOfSpace Hgrad_gin~{idom}~{jdom} ;}
      { Name uD ; Type Local ; NameOfSpace Hgrad_u_Dirichlet2D~{idom} ;}
    }
    Equation {
      ...
      Galerkin { [ -I[]*kDtn[]*{uD}, {gout} ] ;
      	In Sigma~{idom}~{jdom} ; Jacobian JSur ; Integration I1 ; }      
      Galerkin { [ (#17 > 0 ? -{gin} : 0.), {gout} ] ;
      	In Sigma~{idom}~{jdom} ; Jacobian JSur ; Integration I1 ; }      
    }
  }
  { Name Compute_gin~{idom}~{jdom} ; Type FemEquation ;
    Quantity {
      { Name gin ; Type Local ; NameOfSpace Hgrad_gin~{idom}~{jdom} ;}
      { Name gout ; Type Local ; NameOfSpace Hgrad_gout~{idom}~{jdom} ;}
      { Name u ; Type Local ; NameOfSpace Hgrad_u_Dirichlet2D~{idom} ;}
      { Name uPmlDual ; Type Local ; NameOfSpace Hgrad_u_pml_dual~{idom}~{jdom} ;}
      { Name lambda ; Type Local ; NameOfSpace Hgrad_lambda~{idom}~{jdom} ;}
    }
    Equation {
      Galerkin { [ Dof{gin}, {gin} ] ;
      	In Sigma~{idom}~{jdom} ; Jacobian JSur ; Integration I1 ; }
      Galerkin { [ ( #17 > 0 ? -ginn~{idom}~{jdom}[] : 0. ), {gin} ] ; // incremental correction => add previous value
      	In Sigma~{idom}~{jdom} ; Jacobian JSur ; Integration I1 ; }
      Galerkin { [ (1.-alpha)*(-{lambda}+{gout}), {gin} ] ; // primal
      	In Sigma~{idom}~{jdom} ; Jacobian JSur ; Integration I1 ; }
      Galerkin { [ alpha*(I[]*kDtn[]*({uPmlDual}-{u})), {gin} ] ; // dual
      	In Sigma~{idom}~{jdom} ; Jacobian JSur ; Integration I1 ; }      
    }
  }
  ...
  ///////////////////////////////////////////////////////////////////
	...
	If(jdom == 1)
	  ...
	  Galerkin { [ (#11 > 0. ? g_in~{idom}~{1}[] : 0)  , {g_out~{idom}~{1}} ] ;
	    In Sigma~{idom}~{1}; Jacobian JSur ; Integration I1 ; }
	EndIf
	Galerkin { [ -Dof{g_bb~{idom}~{jdom}} , {g_out~{idom}~{jdom}} ] ;
	  In Sigma~{idom}~{jdom}; Jacobian JSur ; Integration I1 ; }
      }
    }
...
PostProcessing {
  For ii In {0: #ListOfDom()-1}
	...
	PostQuantity {
      ...
      // 	}
      // }
      { Name gin~{idom}~{jdom} ; NameOfFormulation Compute_gin~{idom}~{jdom} ;
	Quantity {
	  { Name gin ; Value {
	      Term { [ {gin} ] ; In #{Sigma~{idom}~{jdom}}; Jacobian JSur ; }
	    }
	  }
	}
      }
