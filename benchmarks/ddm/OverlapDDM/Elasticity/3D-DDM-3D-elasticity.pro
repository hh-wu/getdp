...
// The following code takes nx * ny subdomains (defined in "3D-DDM-3D-elasticity.geo") as input and solves a 3D elasticity formulation on the union of all subdomains. It does so by iterating between subdomains using a gmres accelerated DDM-with-overlap method.
...
Function {
	...
	For x In {0:nx-1}
		For y In {0:ny-1}
			...
			g_up~{x+nx*y+1}[] 	= ComplexVectorField[XYZ[]]{100+4*x+4*nx*y+1};
			g_right~{x+nx*y+1}[] 	= ComplexVectorField[XYZ[]]{100+4*x+4*nx*y+2};
			g_down~{x+nx*y+1}[] 	= ComplexVectorField[XYZ[]]{100+4*x+4*nx*y+3};
			g_left~{x+nx*y+1}[]	= ComplexVectorField[XYZ[]]{100+4*x+4*nx*y+4};
		EndFor
	EndFor
...
Constraint	{
	For x In {0:nx-1}
		For y In {0:ny-1}
			{ Name forced_constraints_compx~{x+nx*y+1} ; 
				Case {
					// Constraint on sigma_up~{x+nx*y+1}
					If (y < ny-1)	
		      				{ Region sigma_up~{x+nx*y+1}; Value (#10 > 0 ? CompX[g_up~{x+nx*y+1}[]] : 0 ); }	
					EndIf
					//If (y == ny-1)			
		      			//	{ Region sigma_up~{x+nx*y+1}; Value (#9 > 0 ? 0.1 : 0 ); }
					//EndIf
					// Constraint on sigma_right~{x+nx*y+1}
					If (x < nx-1)			
		      				{ Region sigma_right~{x+nx*y+1}; Value (#10 > 0 ? CompX[g_right~{x+nx*y+1}[]] : 0 ); }
					EndIf
					//If (x == nx-1)			
					...
					If (y > 0)			
						{ Region sigma_down~{x+nx*y+1}; Value (#10 > 0 ? CompX[g_down~{x+nx*y+1}[]] : 0 ); }
					EndIf
					//If (y == 0)			
					//	{ Region sigma_down~{x+nx*y+1}; Value (#9 > 0 ? -0.1 : 0 ); }
					//EndIf
					// Constraint on sigma_left~{x+nx*y+1}
					If (x > 0)			
						{ Region sigma_left~{x+nx*y+1}; Value (#10 > 0 ? CompX[g_left~{x+nx*y+1}[]] : 0 ); }
					EndIf
					If (x == 0)			
			...
			{ Name forced_constraints_compy~{x+nx*y+1} ; 
				Case {
					// Constraint on sigma_up~{x+nx*y+1}
					If (y < ny-1)	
		      				{ Region sigma_up~{x+nx*y+1}; Value (#10 > 0 ? CompY[g_up~{x+nx*y+1}[]] : 0 ); }	
					EndIf
					//If (y == ny-1)			
					...
					// Constraint on sigma_right~{x+nx*y+1}
					If (x < nx-1)			
		      				{ Region sigma_right~{x+nx*y+1}; Value (#10 > 0 ? CompY[g_right~{x+nx*y+1}[]] : 0 ); }
					EndIf
					//If (x == nx-1)			
					//	{ Region sigma_right~{x+nx*y+1}; Value (#9 > 0 ? 0 : 0 ); }
					//EndIf
					// Constraint on sigma_down~{x+nx*y+1}
					If (y > 0)			
						{ Region sigma_down~{x+nx*y+1}; Value (#10 > 0 ? CompY[g_down~{x+nx*y+1}[]] : 0 ); }
					EndIf
					//If (y == 0)			
					...
					// Constraint on sigma_left~{x+nx*y+1}
					If (x > 0)			
						{ Region sigma_left~{x+nx*y+1}; Value (#10 > 0 ? CompY[g_left~{x+nx*y+1}[]] : 0 ); }
					EndIf
					If (x == 0)			
						{ Region sigma_left~{x+nx*y+1}; Value (#9 > 0 ? 0 : 0 ); }
					EndIf
			...
			{ Name forced_constraints_compz~{x+nx*y+1} ; 
				Case {
					// Constraint on sigma_up~{x+nx*y+1}
					If (y < ny-1)	
		      				{ Region sigma_up~{x+nx*y+1}; Value (#10 > 0 ? CompZ[g_up~{x+nx*y+1}[]] : 0 ); }	
					EndIf
					//If (y == ny-1)			
		      			//	{ Region sigma_up~{x+nx*y+1}; Value (#9 > 0 ? 0 : 0 ); }
					//EndIf
					// Constraint on sigma_right~{x+nx*y+1}
					If (x < nx-1)			
		      				{ Region sigma_right~{x+nx*y+1}; Value (#10 > 0 ? CompZ[g_right~{x+nx*y+1}[]] : 0 ); }
					EndIf
					If (x == nx-1)			
						{ Region sigma_right~{x+nx*y+1}; Value (#9 > 0 ? 0.5 : 0 ); }
					EndIf
					// Constraint on sigma_down~{x+nx*y+1}
					If (y > 0)			
						{ Region sigma_down~{x+nx*y+1}; Value (#10 > 0 ? CompZ[g_down~{x+nx*y+1}[]] : 0 ); }
					EndIf
					//If (y == 0)			
					//	{ Region sigma_down~{x+nx*y+1}; Value (#9 > 0 ? 0 : 0 ); }
					//EndIf
					// Constraint on sigma_left~{x+nx*y+1}
					If (x > 0)			
						{ Region sigma_left~{x+nx*y+1}; Value (#10 > 0 ? CompZ[g_left~{x+nx*y+1}[]] : 0 ); }
					EndIf
					If (x == 0)			
						{ Region sigma_left~{x+nx*y+1}; Value (#9 > 0 ? 0 : 0 ); }
					EndIf
...
FunctionSpace	{
	For x In {0:nx-1}
		For y In {0:ny-1}
			{ Name omega~{x+nx*y+1}; Type Vector; 
				BasisFunction{
    				...
    				Constraint{
    					{NameOfCoef uxn; EntityType NodesOf; NameOfConstraint forced_constraints_compx~{x+nx*y+1};}
    					{NameOfCoef uyn; EntityType NodesOf; NameOfConstraint forced_constraints_compy~{x+nx*y+1};}
    					{NameOfCoef uzn; EntityType NodesOf; NameOfConstraint forced_constraints_compz~{x+nx*y+1};}
     				}
			}
		EndFor
	EndFor
	...
	For x In {0:nx-1}
		For y In {0:ny-1}		
			{ Name omega~{x+nx*y+1}; Type FemEquation; Quantity{
				{Name u_omega~{x+nx*y+1}; Type Local; NameOfSpace omega~{x+nx*y+1};}}
				Equation{
					 ...
					 In Region[omega~{x+nx*y+1}] ; Jacobian JVol ; Integration I1 ; }
				       ...
				       // Put a volume/gravity term if you wish, do not forget to multiply it by #9.
				       //Galerkin { [ -1000 * 9.81 * #9 , {u_omega~{x+nx*y+1}} ] ; 
				 	// In Region[omega~{x+nx*y+1}] ; Jacobian JVol ; Integration I1 ; }
				}
			}
		EndFor
...
Resolution	{
		{Name res; 
			System	{
				For x In {0:nx-1}
					For y In {0:ny-1}
						{Name resolution_omega~{x+nx*y+1}; NameOfFormulation omega~{x+nx*y+1}; Type Complex; Frequency freq;}
					EndFor
				EndFor
				}
			Operation{
				...
				IterativeLinearSolver["I-A", "gmres", 1e-4, 200, 200, {101:100+4*nx*ny}, {}, {}]
					{
					SetCommSelf;
					For x In {0:nx-1}
						For y In {0:ny-1}
...
PostProcessing 	{
		For x In {0:nx-1}
			For y In {0:ny-1}
  				...
  				{ Name u_omega~{x+nx*y+1}; NameOfFormulation omega~{x+nx*y+1};
    					Quantity {
      						{ Name u_omega~{x+nx*y+1} ; Value { Local { [ {u_omega~{x+nx*y+1}} ]; 
							In Region[omega~{x+nx*y+1}] ; Jacobian JVol ; } } }
						

      						{ Name g~{x+nx*y+1} ; Value { Local { [ {u_omega~{x+nx*y+1}} ]; 
...
PostOperation	{
		For x In {0:nx-1}
			For y In {0:ny-1}
				...
				{ Name update_g_up~{x+nx*y+1} ; NameOfPostProcessing u_omega~{x+nx*y+1}; 
					Operation {
						Print[ g~{x+nx*y+1}, OnElementsOf sigma_down~{x+nx*(y+1)+1}, 
							StoreInField 100+4*x+4*nx*(y+1)+3] ;	
						}
				}
				{ Name update_g_right~{x+nx*y+1} ; NameOfPostProcessing u_omega~{x+nx*y+1}; 
					Operation {
						Print[ g~{x+nx*y+1}, OnElementsOf sigma_left~{(x+1)+nx*y+1}, 
							StoreInField 100+4*(x+1)+4*nx*y+4] ;	
						}
				}
				{ Name update_g_down~{x+nx*y+1} ; NameOfPostProcessing u_omega~{x+nx*y+1}; 
					Operation {
						Print[ g~{x+nx*y+1}, OnElementsOf sigma_up~{x+nx*(y-1)+1}, 
							StoreInField 100+4*x+4*nx*(y-1)+1] ;	
						}
				}
				{ Name update_g_left~{x+nx*y+1} ; NameOfPostProcessing u_omega~{x+nx*y+1}; 
					Operation {
				...
				{ Name save_omega~{x+nx*y+1} ; NameOfPostProcessing u_omega~{x+nx*y+1}; 
					Operation {
