...
void Geo_GenerateEdgesOfTreeByDimension(int dim, List_T * InitialList, List_T * ExtendedList,
				 Tree_T * EntitiesInTree_T)
{
  int  Nbr_Element, i_Element, Nbr_Entities2, i, Num_Entity1, Dim_Element ;
  struct Geo_Element  * Geo_Element ;
  int  i_Entity2, Num_Entity2, * D_Element, * Entity_P, Entity, Flag_Change ;
  struct EntityInTree  * EntitiesInTree_P[NBR_MAX_ENTITIES_IN_ELEMENT] ;
  struct EntityInTree  EntityInTree_S ;
									  ...
									  Tree_T * EntitiesInTree_T, int SuppListType2)
{
  ...
  for (i_Element = 0 ; i_Element < Nbr_Element ; i_Element++) {
	...
	if (List_Search(InitialList, &Geo_Element->Region, fcmp_int) ) {
	  Geo_GetNodesCoordinates(Geo_Element->NbrNodes, Geo_Element->NumNodes, x, y, z);
  ...
  //std::cout << "Added in tree " << Tree_Nbr(EdgesInTree_T) << "\n";
